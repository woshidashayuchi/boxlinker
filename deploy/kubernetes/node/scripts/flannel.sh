...
while true; do
  /opt/kubernetes/bin/etcdctl --no-sync -C ${ETCD_SERVERS} \
    get /${HOST_NAME}/network/config >/dev/null 2>&1
  if [[ "$?" == 0 ]]; then
    break
  else
    if (( attempt > 600 )); then
    ...
    /opt/kubernetes/bin/etcdctl --no-sync -C ${ETCD_SERVERS} \
      mk /${HOST_NAME}/network/config "{\"Network\":\"${FLANNEL_NET}\"}" >/dev/null 2>&1
    attempt=$((attempt+1))
    sleep 3
  fi
done
