...
class MonitorAct(object):
    ...
    def nape(self, json_data):
        ...
        if rtype == "cpu":
            result.append("cpu/node_utilization")
        ...
        if rtype == "memory":
            result.append("memory/node_utilization")
    ...
    def struct(self, json_data):
        ...
        if rtype == "memory":
            ...
            if node_name is None or node_name == "":
                ...
                return STATICFONT+m_sql
        if rtype == "filesystem":
            STATICFILESYSTEM = "SELECT sum(\"value\") FROM " \
    ...
    def struct_admin(self, json_data):
        node_name = json_data.get("node_name")
        time_long = json_data.get("time_long")
        rtype = json_data.get("rtype")
        result = json_data.get("result")
        ...
        if rtype == "cpu":
            ...
            STATICCPU = "SELECT sum(\"value\") FROM \"%s\" WHERE \"type\" = 'node' " \
                        "AND \"nodename\" =~ /node_name$/ " \
                        "AND time > now() - %s GROUP BY time(1m), \"nodename\" fill(null);" \
                        "SELECT sum(\"value\") FROM \"%s\" WHERE \"type\" = 'node' " \
                        "AND \"nodename\" =~ /node_name$/ " \
                        "AND time > now() - %s GROUP BY time(1m), \"nodename\" fill(null);" \
                        "SELECT sum(\"value\") FROM \"%s\" WHERE \"type\" = 'node' " \
                        "AND \"nodename\" =~ /node_name$/ " \
                        ...
                        "AND \"nodename\" =~ /node_name$/ " \
                        "AND time > now() - %s GROUP BY time(1m), \"nodename\" fill(null)" \
                        "&epoch=ms" % (result[0], time_long, result[1], time_long,
        ...
        if rtype == "memory":
            STATICMEMORY = "SELECT sum(\"value\") " \
                           ...
                           "AND time > now() - %s GROUP BY time(1m), " \
                           "\"nodename\" fill(null)" \
                           "&epoch=ms " % (result[0], time_long, result[1], time_long, result[2],
                                           time_long, result[3], time_long, result[4], time_long)
            if node_name is None or node_name == "":
                m_sql = STATICMEMORY.replace("AND \"nodename\" =~ /node_name$/", "")
    ...
    def count_msg(self, json_data):
        ...
        if json_data.get("app") == "rest":
            url = self.struct_admin(json_data)
        else:
