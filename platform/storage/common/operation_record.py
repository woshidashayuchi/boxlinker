...
from security_rpcapi import rpc_api as security_rpcapi
...
def operation_record(resource_type, action):
    ...
    def _retinfo(func):
        ...
        def __retinfo(*args, **kwargs):
            ...
            security_rpcapi = security_rpcapi.SecurityRpcApi()
            ...
            if token is None:
                ...
                log.critical('context=%s' % (context))
            ...
            try:
                ...
                log.error('function(%s) exec error, reason=%s' %(func.__name__, e))
