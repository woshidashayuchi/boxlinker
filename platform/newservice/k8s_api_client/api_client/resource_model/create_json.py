...
from base_call import BaseCallForOthers
...
class SourceModel(object):
    ...
    def update_service(self, json_list):
        ...
        user_name = json_list.get("user_name")
        container = json_list.get("container")
        domain_ab = BaseCallForOthers.for_container_update(json_list)
        try:
            http_lb, tcp_lb, tcp_port = ArrayIterator.service_domain(json_list)
        except Exception, e:
            log.error("the domain create error, reason=%s"
        ...
        try:
            ...
            if json_list.get("type") == "container":
                containerPort = json_list.get("containerPort")
                if containerPort is not None and containerPort != "":
                    log.info("test......")
                    update_service["metadata"]["annotations"]["serviceloadbalancer/lb.http"] = json_list.get("domain")
            '''
        except Exception, e:
            log.error("service create json error reason=%s" % e)
            return
