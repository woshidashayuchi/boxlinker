...
class FontService(object):
    ...
    def __repr__(self):
        return '%s(%r, %r, %r, %r, %r, %r, %r, %r, %r, %r, %r)' % \
               (self.__class__.__name__, self.uuid, self.user_id, self.orga_id, self.service_id, self.rc_id,
                self.fservice_name, self.fservice_status, self.all_name, self.image_dir, self.fservice_create_time,
                self.fservice_update_time)
