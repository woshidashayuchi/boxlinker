...
class CallBilling(object):
    ...
    @classmethod
    def billing_delete(cls, json_data):
        uuid = json_data.get("uid_font")
        token = json_data.get("token")
        url = "http://%s/%s" % (os.environ.get("BILLING"), uuid)
        headers = {"token": token}
        result = requests.delete(url, headers=headers)
        log.info("headers type is =====%s,data is =====%s, uuid=====%s, url====%s" % (type(result), result, uuid, url
                                                                                      ))
