...
from driver import appstatus_driver
...
class AppStatusManager(object):
    ...
    def __init__(self):
        ...
        self.k8s_driver = appstatus_driver.K8sDriver()
    ...
    def rc_status_update(self, rc_status_cache):
        ...
        for rc_info in rc_status_list:
            try:
                ...
                pod_name = rc_info['metadata']['labels'].get('component')
                if rc_name is None or pod_name is None:
                    continue
            ...
            if rc_name not in rc_status_new.keys():
                ...
                rc_status_new[rc_name]['pods_info'].append(
                    {pod_name: pod_status, project_uuid: project_uuid})

        rc_off_list = dict.fromkeys(
                      [x for x in rc_status_cache if x not in rc_status_new]
                      ).keys()
        for rc_off_name, project_uuid in rc_off_list:
