...
class BillingDB(MysqlInit):
    ...
    def resource_update(self, resource_uuid, resource_conf=None,
                        resource_status=None, resource_orga=None,
                        resource_user=None):
        ...
        sql_03 = "update resources_acl set orga_uuid='%s', update_time=now() \
                  where resource_uuid='%s'" \
                  % (resource_orga, resource_uuid)

        sql_04 = "update resources_acl set user_uuid='%s', update_time=now() \
                  where resource_uuid='%s'" \
                  % (resource_user, resource_uuid)
    ...
    def resource_get(self, user_uuid, orga_uuid, role_uuid):
        ...
        if (role_uuid/100 == 1):
            sql = "select a.resource_uuid, a.resource_type, a.admin_uuid, \
                   ...
                   where a.resource_uuid=b.resource_uuid and a.admin_uuid='%s'" \
                   % (user_uuid)
            ...
            sql = "select a.resource_uuid, a.resource_type, a.admin_uuid, \
                   ...
                   b.resource_name, b.resource_conf, b.resource_status \
                   from resources_acl a join resources b \
                   where a.resource_uuid=b.resource_uuid and a.orga_uuid='%s'" \
                   % (orga_uuid)
    ...
    def balance_insert(self, user_uuid, orga_uuid):

        sql = "insert into balances(user_uuid, orga_uuid, \
               balance, update_time) \
               values('%s', '%s', 0, now())" \
               % (user_uuid, orga_uuid)
    ...
    def balance_update(self, user_uuid, orga_uuid, amount):
        ...
        sql = "update balances set balance=balance + %f, update_time=now() \
               where user_uuid='%s' and orga_uuid='%s'" \
               % (amount, user_uuid, orga_uuid)
    ...
    def balance_get(self, user_uuid, orga_uuid):
        ...
        sql = "select balance, update_time \
               from balances where user_uuid='%s' and orga_uuid='%s'" \
               % (user_uuid, orga_uuid)

        return super(BillingDB, self).exec_select_sql(sql)

    def bill_insert(self, user_uuid, orga_uuid,
                    resource_uuid, resource_cost,
                    voucher_cost):

        sql = "insert into bills(resource_uuid, resource_cost, voucher_cost, \
               user_uuid, orga_uuid, insert_time) \
               values('%s', '%f', '%f', '%s', '%s', now())" \
    ...
    def order_insert(self, user_uuid, orga_uuid, order_uuid,
                     resource_uuid, cost, status):

        sql_01 = "insert into resources_acl(resource_uuid, resource_type, \
                  admin_uuid, orga_uuid, user_uuid, create_time, update_time) \
                  values('%s', 'order', '0', '%s', '%s', now(), now())" \
                  % (order_uuid, orga_uuid, user_uuid)
    ...
    def order_get(self, user_uuid, orga_uuid,
        ...
        sql = "select a.resource_uuid, b.resource_uuid, b.cost, \
               ...
               where a.resource_uuid=b.orders_uuid and a.orga_uuid='%s' \
               and a.user_uuid='%s' and b.create_time between '%s' and '%s'" \
               % (orga_uuid, user_uuid, start_time, end_time)
