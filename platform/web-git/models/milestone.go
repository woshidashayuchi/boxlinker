...
type Milestone struct {
	...
	RelAvatarLink  string    `xorm:"-"`
	HomeLink       string    `xorm:"-"`
	CreateTimeFor  time.Time `xorm:"-"`
}
...
func (m *Milestone) CountIssues(isClosed int8, includePulls bool) int64 {
...
func GetMilestonesUser(userid int64) (*User, error) {
	return getUserByID(x, userid)
}

// GetMilestones returns a list of milestones of given repository and status.
func GetMilestones(repoID int64, page int, isClosed int8) ([]*Milestone, error) {
...
func countRepoFixMilestonse(e Engine, repoID int64) int64 {
	fix, _ := e.Where("repo_id=? AND is_closed=?", repoID, 2).Count(new(Milestone))
	return fix
}

func CountRepoFixMilestones(repoID int64) int64 {
	return countRepoFixMilestonse(x, repoID)
}

// MilestoneStats returns number of open and closed milestones of given repository.
func MilestoneStats(repoID int64) (open int64, closed int64, fix int64) {
	open, _ = x.Where("repo_id=? AND is_closed=?", repoID, false).Count(new(Milestone))
	return open, CountRepoClosedMilestones(repoID), CountRepoFixMilestones(repoID)
}
