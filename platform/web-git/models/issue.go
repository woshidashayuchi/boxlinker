...
type Issue struct {
	...
	AssigneeID      int64
	Assignee        *User        `xorm:"-"`
	IsClosed        int8         // 0 Is Open and 1 Is Closed and 2 Is Fix
	IsRead          bool         `xorm:"-"`
...
func GetRepoName(repoID int64) (string, error) {
...
func (i *Issue) State() api.StateType {
	...
	} else if i.IsClosed == 1 {
		return api.STATE_CLOSED
	} else if i.IsClosed == 2 {
		return api.STATE_FIX
	}
	return "Error"
}
...
func GetStateFromComment(st string) CommentType {
	if st == "reopen" {
		return COMMENT_TYPE_REOPEN
	} else if st == "close" {
		return COMMENT_TYPE_CLOSE
	} else if st == "fix" {
		return COMMENT_TYPE_FIX
	}

	return -1
}

func (i *Issue) changeStatus(e *xorm.Session, doer *User, repo *Repository, isClosed string) (err error) {
	// Nothing should be performed if current status is same as target status
	preparest := GetState(isClosed)

	if i.IsClosed == preparest {
		fmt.Println("equal: ", preparest)
		return nil
	}
	if preparest < 0 {
		return errors.New("State Error")
	}
	i.IsClosed = preparest

	if err = updateIssueCols(e, i, "is_closed"); err != nil {
		fmt.Println(err)
		return err
	} else if err = updateIssueUsersByStatus(e, i.ID, i.IsClosed); err != nil {
		fmt.Println(err)
		return err
	}
	...
	for idx := range i.Labels {
		...
		} else if i.IsClosed == 0 {
			i.Labels[idx].NumClosedIssues--
		} else if i.IsClosed == 2 {
			i.Labels[idx].NumFixedIssues++
		}
	...
	fmt.Println("changeStatus: ", isClosed)
	// New action comment
	if _, err = createStatusComment(e, doer, repo, GetStateFromComment(isClosed), i); err != nil {
		fmt.Println(err)
		return err
	}
...
func (issue *Issue) ChangeStatus(doer *User, repo *Repository, isClosed string) (err error) {
	sess := x.NewSession()
	defer sessionRelease(sess)
	if err = sess.Begin(); err != nil {
		fmt.Println(err)
	...
	if issue.IsPull {
		...
		if api.StateType(isClosed) == api.STATE_OPEN {
			apiPullRequest.Action = api.HOOK_ISSUE_REOPENED
		} else if api.StateType(isClosed) == api.STATE_CLOSED {
			apiPullRequest.Action = api.HOOK_ISSUE_CLOSED
		} else {
			apiPullRequest.Action = api.HOOK_ISSUE_FIXED
		}
		...
		if api.StateType(isClosed) == api.STATE_CLOSED {
			apiIssues.Action = api.HOOK_ISSUE_CLOSED
		} else if api.StateType(isClosed) == api.STATE_OPEN {
			apiIssues.Action = api.HOOK_ISSUE_REOPENED
		} else if api.StateType(isClosed) == api.STATE_FIX {
			apiIssues.Action = api.HOOK_ISSUE_FIXED
		}
...
func (issue *Issue) ChangeImportance(doer *User, importanceID int8) (err error) {
	issue.Importance = importanceID
	if err = UpdateIssueUserByImportance(issue); err != nil {
		return fmt.Errorf("UpdateIssueUserByImportance: %v", err)
	}
...
func GetIssueUsers(rid, uid int64, isClosed int8) ([]*IssueUser, error) {
...
func GetIssueUserPairsByRepoIds(rids []int64, isClosed int8, page int) ([]*IssueUser, error) {
	if len(rids) == 0 {
...
func GetIssueUserPairsByMode(userID, repoID int64, filterMode FilterMode, isClosed int8, page int) ([]*IssueUser, error) {
	ius := make([]*IssueUser, 0, 10)
...
type IssueStats struct {
	OpenCount, ClosedCount, FixCount int64
...
func GetIssueByAssignee(assigneeid int64) *IssueStats {
	stats := &IssueStats{}

	countSession := func(isClosed int8) *xorm.Session {
		sess := x.Where("issue.is_closed = ?", isClosed)
		return sess
	}

	stats.OpenCount, _ = countSession(0).And("assignee_id = ?", assigneeid).Count(new(Issue))
	return stats
}
...
func GetIssueStats(opts *IssueStatsOptions) *IssueStats {
	...
	case FILTER_MODE_YOUR_REPOS, FILTER_MODE_ASSIGN:
		...
		stats.FixCount, _ = countSession(opts).
			And("is_closed = ?", 2).
			Count(new(Issue))
	case FILTER_MODE_CREATE:
		stats.OpenCount, _ = countSession(opts).
...
func GetUserIssueStats(repoID, userID int64, repoIDs []int64, filterMode FilterMode, isPull bool) *IssueStats {
	...
	countSession := func(isClosed int8, isPull bool, repoID int64, repoIDs []int64) *xorm.Session {
	...
	stats.AssignCount, _ = countSession(0, isPull, repoID, nil).
		And("assignee_id = ?", userID).
		Count(new(Issue))

	stats.CreateCount, _ = countSession(0, isPull, repoID, nil).
		And("poster_id = ?", userID).
		Count(new(Issue))

	if hasAnyRepo {
		stats.YourReposCount, _ = countSession(0, isPull, repoID, repoIDs).
			Count(new(Issue))
	}
	...
	case FILTER_MODE_YOUR_REPOS:
		...
		stats.OpenCount, _ = countSession(0, isPull, repoID, repoIDs).
			Count(new(Issue))
		stats.ClosedCount, _ = countSession(1, isPull, repoID, repoIDs).
			Count(new(Issue))
	case FILTER_MODE_ASSIGN:
		stats.OpenCount, _ = countSession(0, isPull, repoID, nil).
			And("assignee_id = ?", userID).
			Count(new(Issue))
		stats.ClosedCount, _ = countSession(1, isPull, repoID, nil).
			And("assignee_id = ?", userID).
			Count(new(Issue))
	case FILTER_MODE_CREATE:
		stats.OpenCount, _ = countSession(0, isPull, repoID, nil).
...
func updateIssueUsersByStatus(e Engine, issueID int64, isClosed int8) error {
...
func UpdateIssueUsersByStatus(issueID int64, isClosed int8) error {
...
func UpdateIssueUserByImportance(issue *Issue) error {
	_, err := x.Exec("UPDATE `issue` SET importance =? WHERE issue.index=?", issue.Importance, issue.Index)
	return err
}

func DeleteIssue(issue *Issue) (err error) {
	sess := x.NewSession()
	defer sessionRelease(sess)
	if err = sess.Begin(); err != nil {
		return err
	...
	if _, err = sess.Delete(issue); err != nil {
		return err
	}
