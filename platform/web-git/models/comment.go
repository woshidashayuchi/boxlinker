...
import (
	...
	"github.com/cabernety/boxlinker-git/modules/markdown"
)
...
const (
	...
	// Assign COMMENT LIKE CLOSE
	COMMENT_TYPE_ASSIGN
	// Change Importance COMMENT
	COMMENT_TYPE_IMPORTANCE
	// Lable COMMENT
	COMMENT_TYPE_LABEL
	// Milestone COMMENT
...
type Comment struct {
	...
	AssigneeId     int64
	AssigneeName   string `xorm:"-"`
	AssigneeAvatar string `xorm:"-"`
	AssigneeLink   string `xorm:"-"`

	Importance int8

	Label    int64
	AddOrDel int8 // 1 is add and -1 is del default: 0
...
func (cmt *Comment) mailParticipants(e Engine, opType ActionType, issue *Issue) (err error) {
	...
	case ACTION_FIX_ISSUE:
		issue.Content = fmt.Sprintf("Fixed #%d", issue.Index)
	}
...
func createComment(e *xorm.Session, opts *CreateCommentOptions) (_ *Comment, err error) {
	comment := &Comment{
		...
		Label:      opts.Label,
		AddOrDel:   opts.AddOrDel,
		Milestone:  opts.Milestone,
	}
	fmt.Println("createComment type: ", comment.Type)
	...
	case COMMENT_TYPE_FIX:
		act.OpType = ACTION_FIX_ISSUE
		if opts.Issue.IsPull {
			act.OpType = ACTION_CLOSE_PULL_REQUEST
		}
	case COMMENT_TYPE_MILESTONE:
		act.OpType = ACTION_SET_MILESTONE
		if opts.Issue.IsPull {
			act.OpType = ACTION_CLOSE_PULL_REQUEST
		}
	case COMMENT_TYPE_LABEL:
		act.OpType = ACTION_CHANGE_LABEL
		if opts.Issue.IsPull {
			act.OpType = ACTION_CLOSE_PULL_REQUEST
		}
	case COMMENT_TYPE_IMPORTANCE:
		act.OpType = ACTION_IMPORTANCE_ISSUE
		if opts.Issue.IsPull {
			act.OpType = ACTION_CLOSE_PULL_REQUEST
		}
	case COMMENT_TYPE_ASSIGN:
		act.OpType = ACTION_ASSIGNEE_ISSUE
		if opts.Issue.IsPull {
			act.OpType = ACTION_CLOSE_PULL_REQUEST
		}
	}
...
func createStatusComment(e *xorm.Session, doer *User, repo *Repository, isclosed CommentType, issue *Issue) (*Comment, error) {
	cmtType := COMMENT_TYPE_CLOSE
	if isclosed == COMMENT_TYPE_REOPEN {
		cmtType = COMMENT_TYPE_REOPEN
	} else if isclosed == COMMENT_TYPE_CLOSE {
		cmtType = COMMENT_TYPE_CLOSE
	} else if isclosed == COMMENT_TYPE_FIX {
		cmtType = COMMENT_TYPE_FIX
	}
...
func CreateAssigneeComment(doer *User, repo *Repository, issue *Issue, assignee int64) error {
	fmt.Println("CreateAssigneeComment: ", assignee)
	_, err := CreateComment(&CreateCommentOptions{
		Type:       COMMENT_TYPE_ASSIGN,
		Doer:       doer,
		Repo:       repo,
		Issue:      issue,
		AssigneeId: assignee,
...
func CreateImportanceComment(doer *User, repo *Repository, issue *Issue, imp int8) error {
	_, err := CreateComment(&CreateCommentOptions{
		Type:       COMMENT_TYPE_IMPORTANCE,
		Doer:       doer,
		Repo:       repo,
		Issue:      issue,
		Importance: imp,
...
func CreateLabelComment(doer *User, repo *Repository, issue *Issue, la int64, addordel int8) error {
	_, err := CreateComment(&CreateCommentOptions{
...
func CreateMilestoneComment(doer *User, repo *Repository, issue *Issue, mi int64) error {
	_, err := CreateComment(&CreateCommentOptions{
