...
const (
	...
	ACTION_FORK_REPO                                 // 19
	ACTION_FIX_ISSUE                                 // 20
	ACTION_ASSIGNEE_ISSUE                            // 21
	ACTION_IMPORTANCE_ISSUE                          // 22
	ACTION_CHANGE_LABEL                              // 23
	ACTION_SET_MILESTONE                             // 24
	ACTION_DELETE_ISSUE                              // 25
)
...
type Action struct {
	...
	Milestone     int64
	LabelInfo     *Label     `xorm:"-"`
	MilestoneInfo *Milestone `xorm:"-"`
}
...
func newDeleteIssueAction(e Engine, doer, owner *User, repo *Repository) (err error) {
	return notifyWatchers(e, &Action{
		ActUserID:    doer.ID,
		ActUserName:  doer.Name,
		OpType:       ACTION_DELETE_ISSUE,
		RepoID:       repo.ID,
		RepoUserName: repo.Owner.Name,
		RepoName:     repo.Name,
		IsPrivate:    repo.IsPrivate,
...
// NewDeleteIssueAction adds new action for delete issue
func NewDeleteIssueAction(doer, owner *User, repo *Repository) (err error) {
	return newDeleteIssueAction(x, doer, owner, repo)
}
...
func UpdateIssuesCommit(doer *User, repo *Repository, commits []*PushCommit) error {
	...
	for i := len(commits) - 1; i >= 0; i-- {
		...
		for _, ref := range IssueCloseKeywordsPat.FindAllString(c.Message, -1) {
			...
			if issue.RepoID != repo.ID || issue.IsClosed == 1 {
				continue
			}

			if err = issue.ChangeStatus(doer, repo, "closed"); err != nil {
				return err
			}
		}
		...
		for _, ref := range IssueReopenKeywordsPat.FindAllString(c.Message, -1) {
			...
			if issue.RepoID != repo.ID || issue.IsClosed == 0 {
				continue
			}

			if err = issue.ChangeStatus(doer, repo, "open"); err != nil {
				return err
			}
		}
	}
...
func GetActionWithTime(ctxUser *User, actorID int64, afterTime int64) ([]*Action, error) {
	actions := make([]*Action, 0, setting.UI.User.NewsFeedPagingNum)
	sess := x.Where("user_id = ? ", ctxUser.ID).Asc("created_unix")
	if afterTime > 0 {
		sess.And("created_unix > ?", afterTime-5)
	}
	if actorID != -1 && ctxUser.IsOrganization() {
		repos, _, err := ctxUser.GetUserRepositories(actorID, 1, ctxUser.NumRepos)
		if err != nil {
			return nil, fmt.Errorf("GetUserRepositories: %v", err)
		}
