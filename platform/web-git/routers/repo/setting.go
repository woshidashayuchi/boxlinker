...
func CheckUser(token string, username string) bool {
	url := "https://ucenter.boxlinker.com/api/v1.0/ucenter/users?user_name=" + username + "&name_check=true"
	client := &http.Client{}
...
func SettingsCollaborationPost(ctx *context.Context) {
	...
	if len(name) == 0 || ctx.Repo.Owner.LowerName == name || token == nil {
	...
	if err != nil {
		result := CheckUser(token.Value, name)
		if result == true {
			u = &models.User{
				Name:     name,
				Email:    name + string("@boxlinker.com"),
			...
			if err := models.CreateUser(u); err != nil {
				...
				case models.IsErrUserAlreadyExist(err):
					fmt.Println("username already exist: ", name)
				case models.IsErrEmailAlreadyUsed(err):
					fmt.Println("email already exist: ", name+string("@boxlinker.com"))
				case models.IsErrNameReserved(err):
