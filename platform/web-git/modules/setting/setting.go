...
func NewContext() {
	...
	u := os.Getenv("appurl")
	if u == "" {
		AppUrl = "http://git.boxlinker.com:3000/"
	} else {
		AppUrl = u
	}
	...
	InstallLock = true //sec.Key("INSTALL_LOCK").MustBool()
	...
	if InstallLock {
		/*currentUser*/_, match := IsRunUserMatchCurrentUser(RunUser)
		if !match {
			//log.Error(2, "Expect user '%s' but current user is: %s", RunUser, currentUser)
		}
	}
