...
func (cnf *Configurator) AddOrUpdateIngress(name string, ingEx *IngressEx) {
	...
	pems, isSSL := cnf.updateCertificates(ingEx)
	nginxCfg := cnf.generateNginxCfg(ingEx, pems, isSSL)
...
func (cnf *Configurator) updateCertificates(ingEx *IngressEx) (SSL, bool) {
	ssl := SSL{}
	var result bool
	for _, tls := range ingEx.Ingress.Spec.TLS {
		...
		crtFileName, keyFileName := cnf.nginx.AddOrUpdateCertAndKey(name, string(cert), string(key))
		ssl.key = keyFileName
		ssl.crt = crtFileName
...
func (cnf *Configurator) generateNginxCfg(ingEx *IngressEx, pems SSL, isSSL bool) IngressNginxConfig {
	...
	for _, rule := range ingEx.Ingress.Spec.Rules {
		...
		if isSSL {
			server.SSL = true
			server.SSLCertificate = pems.crt
			server.SSLCertificateKey = pems.key
		}
		...
		for _, path := range rule.HTTP.Paths {
			...
			loc := createLocation(pathOrDefault(path.Path), upstreams[upsName], &ingCfg, wsServices[path.Backend.ServiceName],
				rewrites[path.Backend.ServiceName], sslServices[path.Backend.ServiceName])
		...
		if rootLocation == false && ingEx.Ingress.Spec.Backend != nil {
			...
			loc := createLocation(pathOrDefault("/"), upstreams[upsName], &ingCfg, wsServices[ingEx.Ingress.Spec.Backend.ServiceName],
				rewrites[ingEx.Ingress.Spec.Backend.ServiceName], sslServices[ingEx.Ingress.Spec.Backend.ServiceName])
			locations = append(locations, loc)
		}
	...
	if len(ingEx.Ingress.Spec.Rules) == 0 && ingEx.Ingress.Spec.Backend != nil {
		...
		loc := createLocation(pathOrDefault("/"), upstreams[upsName], &ingCfg, wsServices[ingEx.Ingress.Spec.Backend.ServiceName], rewrites[ingEx.Ingress.Spec.Backend.ServiceName],
			sslServices[ingEx.Ingress.Spec.Backend.ServiceName])
		locations = append(locations, loc)
...
func getNameForUpstream(ing *extensions.Ingress, host string, service string) string {
	return fmt.Sprintf("%v_%v_com", service, ing.Namespace)
}
...
func (cnf *Configurator) generateNginxConfigmap(cfgEx *ConfigmapEx) ConfigMapNginxConfig {
	...
	isUDP := cfgEx.Config.Data["udp"] == "true"
	server := Server{Name: emptyHost, ServerPort: port, ProxyProtocol: isUDP}
