...
class DeviceManager(object):
    ...
    @staticmethod
    def metal_work(all_cpu_data, all_memory_data, all_network_data, all_disk_data, all_lb_data):
        [q.update(w) for q in all_cpu_data for w in all_memory_data if q.get('ip') == w.get('ip')]
        [q.update(w) for q in all_cpu_data for w in all_network_data if q.get('ip') == w.get('ip')]
        [q.update(w) for q in all_cpu_data for w in all_disk_data if q.get('ip') == w.get('ip')]
    ...
    def cpu_data_to_db(self):
        ...
        try:
            to_db_data = self.metal_work(all_cpu_data, all_memory_data, all_network_data, all_disk_data, all_lb_data)
        except Exception, e:
